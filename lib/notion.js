import { Client } from "@notionhq/client";
export const notion = new Client({ auth: process.env.NOTION_TOKEN });
export const DATABASE_ID = process.env.NOTION_DATABASE_ID;

// Quarter auto-assignment based on current date
export function getCurrentQuarter() {
  const m = new Date().getMonth() + 1;
  const y = new Date().getFullYear();
  const q = m <= 3 ? "Q1" : m <= 6 ? "Q2" : m <= 9 ? "Q3" : "Q4";
  return `${q} ${y}`;
}

// Quarter from deadline date
export function getQuarterFromDate(dateStr) {
  if (!dateStr) return getCurrentQuarter();
  const d = new Date(dateStr + "T12:00:00");
  const m = d.getMonth() + 1;
  const y = d.getFullYear();
  const q = m <= 3 ? "Q1" : m <= 6 ? "Q2" : m <= 9 ? "Q3" : "Q4";
  return `${q} ${y}`;
}

export function pageToActivity(page) {
  const p = page.properties;
  const get = (key, type) => {
    const prop = p[key];
    if (!prop) return "";
    if (type === "title") return prop.title?.[0]?.plain_text || "";
    if (type === "select") return prop.select?.name || "";
    if (type === "rich_text") return prop.rich_text?.[0]?.plain_text || "";
    if (type === "number") return prop.number ?? "";
    if (type === "date") return prop.date?.start || "";
    return "";
  };
  return {
    id: page.id,
    Atividade: get("Atividade", "title"),
    Categoria: get("Categoria", "select"),
    Etapa: get("Etapa", "select"),
    Status: get("Status", "select"),
    "Cliente Final": get("Cliente Final", "rich_text"),
    Integrador: get("Integrador", "rich_text"),
    Estado: get("Estado", "select"),
    Região: get("Região", "select"),
    DSI: get("DSI", "select"),
    "Tipo Atividade": get("Tipo Atividade", "select"),
    "Tipo Treinamento": get("Tipo Treinamento", "select"),
    "Nome Treinamento": get("Nome Treinamento", "rich_text"),
    "Treinamento ID": get("Treinamento ID", "select"),
    Quarter: get("Quarter", "select"),
    "Valor (R$)": get("Valor (R$)", "number"),
    "Meta Quarter (R$)": get("Meta Quarter (R$)", "number"),
    Meta: get("Meta", "number"),
    Realizado: get("Realizado", "number"),
    Data: get("Data", "date"),
    Deadline: get("Deadline", "date"),
    Observações: get("Observações", "rich_text"),
  };
}

export function activityToProperties(data) {
  const props = {};
  if (data.Atividade !== undefined) props["Atividade"] = { title: [{ text: { content: data.Atividade || "" } }] };
  if (data.Categoria) props["Categoria"] = { select: { name: data.Categoria } };
  if (data.Etapa) props["Etapa"] = { select: { name: data.Etapa } };
  else props["Etapa"] = { select: null };
  if (data.Status) props["Status"] = { select: { name: data.Status } };
  if (data["Cliente Final"] !== undefined) props["Cliente Final"] = { rich_text: [{ text: { content: data["Cliente Final"] || "" } }] };
  if (data.Integrador !== undefined) props["Integrador"] = { rich_text: [{ text: { content: data.Integrador || "" } }] };
  if (data.Estado) props["Estado"] = { select: { name: data.Estado } };
  else props["Estado"] = { select: null };
  if (data.Região) props["Região"] = { select: { name: data.Região } };
  else props["Região"] = { select: null };
  if (data.DSI) props["DSI"] = { select: { name: data.DSI } };
  else props["DSI"] = { select: null };
  if (data["Tipo Atividade"]) props["Tipo Atividade"] = { select: { name: data["Tipo Atividade"] } };
  else props["Tipo Atividade"] = { select: null };
  if (data["Tipo Treinamento"]) props["Tipo Treinamento"] = { select: { name: data["Tipo Treinamento"] } };
  else props["Tipo Treinamento"] = { select: null };
  if (data["Nome Treinamento"] !== undefined) props["Nome Treinamento"] = { rich_text: [{ text: { content: data["Nome Treinamento"] || "" } }] };
  if (data["Treinamento ID"]) props["Treinamento ID"] = { select: { name: data["Treinamento ID"] } };
  else props["Treinamento ID"] = { select: null };
  if (data.Quarter) props["Quarter"] = { select: { name: data.Quarter } };
  if (data["Valor (R$)"] !== undefined && data["Valor (R$)"] !== "") props["Valor (R$)"] = { number: parseFloat(data["Valor (R$)"]) || null };
  if (data["Meta Quarter (R$)"] !== undefined && data["Meta Quarter (R$)"] !== "") props["Meta Quarter (R$)"] = { number: parseFloat(data["Meta Quarter (R$)"]) || null };
  if (data.Meta !== undefined && data.Meta !== "") props["Meta"] = { number: parseFloat(data.Meta) || null };
  if (data.Realizado !== undefined && data.Realizado !== "") props["Realizado"] = { number: parseFloat(data.Realizado) || null };
  if (data.Data) props["Data"] = { date: { start: data.Data } };
  if (data.Deadline) props["Deadline"] = { date: { start: data.Deadline } };
  else if (data.Deadline === "") props["Deadline"] = { date: null };
  if (data.Observações !== undefined) props["Observações"] = { rich_text: [{ text: { content: data.Observações || "" } }] };
  return props;
}
